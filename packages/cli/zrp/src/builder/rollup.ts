import { rollup, defineConfig, nodeResolve, strip,json, commonjs, typescript, nodeExternals,nodePath,defu } from "../library"
...
function getRollupOptions(ctx: BuildContext): RollupOptions {
  ...
  if (ctx.options.plugin.typescript) {
      const conf = {
        ...
        useTsconfigDeclarationDir: ctx.options.declarationDir ? true : false,
        tsconfigDefaults: {
          compilerOptions: {
