...
import { dumpObject, resolveRootDir, tryRequire, consoleLog, resolveInput } from './utils'
 ...
 * @returns
 */
async function getContextOptions(rootDir: string, pkgName = ''): Promise<BuildOptions> {
  rootDir = resolveRootDir(rootDir)
  // Read zrp.config.ts or zrp.config.js
  const buildConfig: BuildConfig = tryRequire('./zrp.config', rootDir)
...
async function calulateEntryBytes(ctx: BuildContext) {
  ...
  for (const entry of ctx.entries.filter((e) => !e.chunk)) {
    ...
    if (entry.chunks?.length) {
      line += `\n${entry.chunks
        .map((p) => {
          const chunk = ctx.entries.find((e) => e.path === p) || ({} as any)
          return chalk.gray(`  └─ ${rPath(p)}${chalk.bold(chunk.bytes ? ` (${prettyBytes(chunk?.bytes)})` : '')}`)
        })
        .join('\n')}`
    }
    if (entry.modules?.length) {
      line += `\n${entry.modules
        ...
        .map((m) => {
          return chalk.gray(`  📦 ${rPath(m.id)}${chalk.bold(m.bytes ? ` (${prettyBytes(m.bytes)})` : '')}`)
        })
        .join('\n')}`
    }
    consola.log(entry.chunk ? chalk.gray(line) : line)
  }
  console.log('Σ Total dist size (byte size):', chalk.cyan(prettyBytes(ctx.entries.reduce((a, e) => a + (e.bytes || 0), 0))))
}
